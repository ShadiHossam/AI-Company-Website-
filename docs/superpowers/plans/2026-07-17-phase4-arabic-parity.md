...
12. **Double-check negations when translating a section's core thesis sentence.** Caught in `custom-ai-development.astro`'s review: "which means they're optimized for no one in particular" was translated dropping the negation, so the Arabic literally said the opposite ("they ARE optimized for someone specific") — inverting the page's opening sales argument. A dropped/misplaced "not"/"لا"/"غير" is easy to miss in fluent-reading translated prose because the sentence still reads naturally, it just means the wrong thing. Pay extra attention to negation words on hero/opening/thesis sentences specifically, since those carry the most weight and errors there are the most consequential.
